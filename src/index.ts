...
export default {
  ...
  async bootstrap({ strapi }) {
    ...
    async function findMagazineIdByUrl(url: string): Promise<Magazine | undefined> {
    ...
    const allMagazines = await strapi.entityService?.findMany('api::magazine.magazine');
    ...
    allMagazines.forEach((magazine) => {
      ...
      if (matchingMapping) {
        ...
        urlToMagazineMap.set(matchingMapping.domain, { id: magazineId, name: matchingMapping.name });
      }
    ...
    for (let i = 0; i < articles.length; i++) {
      if (articles[i].urls && articles[i].urls.length > 0 && articles[i].urls.some(url => !url.magazine)) {
        ...
        for (let k = 0; k < articles[i].urls.length; k++) {
          if (!articles[i].urls[k].magazine) {
            ...
            if (matchedMagazine) {
              ...
              await strapi.db.connection("components_url_original_urls_magazine_links").insert({
